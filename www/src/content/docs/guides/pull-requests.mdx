...
<Aside type="tip" title="These conversations stay inside the pull request">
  The Assistant's thread never appears on your board, backlog or Mission Control — it belongs to the pane. It survives closing the app: reopen the pull request and the conversation is where you left it. It archives itself once the pull request is merged or closed, and after two weeks of silence. **Continue in task** moves it into a full task view when you want the whole window.
</Aside>

## Hand the work to an agent

**Send to agent** asks what you want done, rather than handing an agent a link and hoping:

| Choice | What the agent gets |
| --- | --- |
| **Address review comments** | Every unresolved remark with its file and line, and the instruction to fix them on the branch, then commit and push. The count is on the menu item; it's disabled when nothing is unresolved. |
| **Work on this branch** | The description and the files the change touches — pick it up where it left off. |

Either way it starts by checking the pull request's own branch out, and it becomes an ordinary task on your board — unlike the Assistant, this one is real work with an owner.
